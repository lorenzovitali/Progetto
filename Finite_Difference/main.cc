...
int main(int argc, char* argv[]){
  ...
  std::cout << "alpha: " << fdm_CN.get_alpha() << std::endl;
  std::cout << "dx: " << fdm_CN.get_dx() << std::endl;
  std::cout << "dt: "<< fdm_CN.get_dt() << std::endl;
  std::cout << "k: " << call->get_k() << std::endl;
  //std::cout << "rows: "<< result.size() << std::endl;
  //std::cout << "columns: " << result[0].size() << std::endl;

  std::cout << "\n \n \nSOR \n" << std::endl;
  SOR_EI solver1(N, dx, M, toll, pde);
  //double omega = solver.find_opt();
  Matrice matrice = solver1.solve(1.04);
  ...
  std::cout << "\n \n \nPSOR \n" << std::endl;
  PSOR_CN solver2(N, dx, M, toll, pde);
  //double omega = solver.find_opt();
  Matrice risultato = solver2.solve(1.04);
